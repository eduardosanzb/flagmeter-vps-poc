# Implementation Plan: AI Strategy Prompt Improvements

**Date**: 2026-04-09 **Status**: COMPLETED

## Overview

Upgrade the AI Strategy Assessment Prompt with nine improvements based on a real-world test conversation: integer-only scoring, edge-case probing for high scorers, open-ended close, peer benchmarking, honest disclaimer, ASCII snapshot card output, assessment receipt output, enhanced HTML artifact instructions (with raus.cloud branding and CTAs), and a natural consulting CTA bridge. Extract the prompt from the hardcoded HTML layout into a separate markdown content file for easy editing, and update the Outline source of truth.
...
- Execution phases: 2
- Files affected:
  - `apps/landing/content/ai-strategy-prompt.en.md` — create (new file, the prompt as Hugo content)
  - `apps/landing/layouts/_default/ai-strategy.html` — modify (read prompt from content file instead of hardcoded span, fix copy button bug)
  - Outline document `216ffbe4-11e3-4515-8249-d942973aec98` — update

## Work Units

### WU-1: Create Prompt Content File with All Improvements

**Dependencies**: none

**Context**: The assessment prompt is currently hardcoded as ~85 lines of raw text inside a `<span hidden>` element in the layout template `apps/landing/layouts/_default/ai-strategy.html` (lines 18-101). This makes it hard to edit and impossible to translate independently. We need to extract it into its own Hugo content file at `apps/landing/content/ai-strategy-prompt.en.md`. The layout will then read it via `{{ .Site.GetPage "/ai-strategy-prompt" }}` and access the raw markdown body. The content file needs minimal frontmatter (just `title` and `draft: false`) and the full improved prompt as the markdown body. This file should NOT have a `url:` or its own page — it's a headless content bundle used only by the layout. To prevent Hugo from creating a standalone page for it, set `_build.render` to `never` in frontmatter.

**Files**:
- `apps/landing/content/ai-strategy-prompt.en.md` — create

**Steps**:
1. Create the file `apps/landing/content/ai-strategy-prompt.en.md` with the following content:
...
The markdown body (everything after `---`) should be the complete improved prompt text. Here is the exact text to use:
...
**Verification**: Confirm the file exists at `apps/landing/content/ai-strategy-prompt.en.md`, has the `_build: render: never` frontmatter, and the body starts with "You are an AI strategy assessor" and ends with "asking about the first dimension (Adoption Governance)."

**Rollback**:
- `rm -f apps/landing/content/ai-strategy-prompt.en.md`

---

### WU-2: Update Layout to Read Prompt from Content File + Fix Copy Button

**Dependencies**: WU-1

**Context**: The layout at `apps/landing/layouts/_default/ai-strategy.html` currently has the prompt hardcoded as ~85 lines of raw text inside a `<span id="ai-strategy-prompt" hidden>` element (lines 18-101). We need to replace this with a Hugo template call that reads the prompt from the new content file created in WU-1. Hugo's `.Site.GetPage` function can retrieve any content page, and `.RawContent` gives us the raw markdown without HTML rendering — which is exactly what we need since the `copyPrompt()` JavaScript reads `.textContent`.

Additionally, the `copyPrompt()` function (line 630) resets the button text to `'Copy to clipboard'` but the button initially says `'Copy prompt'` (line 102). Fix this inconsistency.

**Files**:
- `apps/landing/layouts/_default/ai-strategy.html` — modify

**Steps**:
1. Read the current file at `apps/landing/layouts/_default/ai-strategy.html`.

2. Find the `<span id="ai-strategy-prompt" hidden>` block. It starts at line 18 with:
```
        <span id="ai-strategy-prompt" hidden>You are an AI strategy assessor for European tech companies...
```
And the closing `</span>` is at line 101 (the line ends with `...asking about the first dimension (Adoption Governance).</span>`).

Replace this entire block (from `<span id="ai-strategy-prompt" hidden>` through `</span>`) with:
```html
        {{ $prompt := .Site.GetPage "/ai-strategy-prompt" }}
        {{ if $prompt }}
        <span id="ai-strategy-prompt" hidden>{{ $prompt.RawContent }}</span>
        {{ end }}
```

This uses Hugo's `.Site.GetPage` to fetch the content file at `/ai-strategy-prompt` (which resolves to `content/ai-strategy-prompt.en.md`), then `.RawContent` to get the raw markdown body as plain text (no HTML conversion). The `hidden` attribute keeps it invisible on page, and the `copyPrompt()` JavaScript reads `.textContent` from it.

3. Find the `copyPrompt()` function (around line 624-632). Find the line:
```javascript
      setTimeout(function() { btn.textContent = 'Copy to clipboard'; }, 2000);
```
Change `'Copy to clipboard'` to `'Copy prompt'`.

**Verification**: `cd apps/landing && hugo --quiet 2>&1; echo $?` exits 0. Then verify key sections are present in the built output by searching `public/ai-strategy/index.html` for: "Score as whole numbers only" (scoring rule), "What could break this" (edge-case probing), "anything about your AI usage that I didn't cover" (open-ended close), "self-reported snapshot, not an audit" (disclaimer), "ASCII Snapshot Card" (output format), "Assessment Receipt" (output format), "Enhanced Visual Artifact" (artifact instructions), "cal.com/eduardosanzb/raus-cloud-audit" (CTA link), "Dialectical Questioning Strategy" (dialectical section), "Copy prompt" (button text appears in both places).
...
### WU-3: Update Outline Prompt Document

**Dependencies**: none

**Context**: The Outline document "AI Strategy Assessment Prompt" (ID: `216ffbe4-11e3-4515-8249-d942973aec98`) is the documentation source of truth. It currently has the basic five-dimension assessment flow plus dialectical questioning but is missing: output format specification, integer scoring rules, edge-case probing, open-ended close, peer benchmarking, disclaimer, and the raus.cloud CTA bridge. We need to update it with the same improved prompt text from WU-1 so Outline and the landing page stay in sync. The Outline document has two parts: (a) an intro paragraph explaining the copy-paste workflow, followed by a `---` separator, then (b) the actual prompt. We preserve the intro and replace the prompt.

**Files**:
- Outline document: "AI Strategy Assessment Prompt" (ID: `216ffbe4-11e3-4515-8249-d942973aec98`) — update

**Steps**:
1. Read the current Outline document via `mcp-outline_read_document` (ID: `216ffbe4-11e3-4515-8249-d942973aec98`).
2. The document starts with a title and intro paragraph ("Copy and paste the prompt below into your AI tool...") followed by a `---` separator. Preserve everything up to and including the `---`.
3. Replace everything after the `---` with the exact same improved prompt text specified in WU-1 (the full text starting with "You are an AI strategy assessor..." and ending with "Begin by introducing the assessment and asking about the first dimension (Adoption Governance).").
4. Update the document via `mcp-outline_update_document` with the complete content (preserved intro + `---` + new prompt).

**Verification**: Re-read the updated Outline document via `mcp-outline_read_document` and confirm: (1) the intro paragraph is preserved, (2) the `---` separator is present, (3) the prompt contains "Score as whole numbers only (1-4)" (integer scoring), (4) "What could break this?" (edge-case probing), (5) "anything about your AI usage that I didn't cover" (open-ended close), (6) "Based on what I typically see in European tech companies" (peer benchmarking), (7) "self-reported snapshot, not an audit" (disclaimer), (8) "ASCII Snapshot Card" (output format), (9) "Assessment Receipt" (output format), (10) "Enhanced Visual Artifact" (artifact instructions), (11) "cal.com/eduardosanzb/raus-cloud-audit" (CTA), (12) "Dialectical Questioning Strategy" section is present.

**Rollback**:
- Re-update the Outline document with the previous content (the current version is preserved in the conversation context above)

---

### WU-4: Add Heart Icon to Slides Icon Partial

**Dependencies**: none

**Context**: The AI Strategy slide deck (slide 5, icon-grid) uses `icon: "heart"` for the Team Health dimension, but the slides icon partial at `apps/landing/layouts/partials/slides-icon.html` does not have a `heart` case — it falls through to a plain circle fallback. We need to add a heart SVG icon to the partial.

**Files**:
- `apps/landing/layouts/partials/slides-icon.html` — modify

**Steps**:
1. Read the file at `apps/landing/layouts/partials/slides-icon.html`.
2. The file contains a series of `{{ if eq .icon "..." }}` blocks followed by `{{ else }}` (the fallback circle). Before the `{{ else }}` fallback block, add a new `{{ else if eq .icon "heart" }}` case with a heart SVG:
```html
{{ else if eq .icon "heart" }}
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-full h-full"><path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"/></svg>
```
3. The heart icon SVG follows the same pattern as other icons in the file (Lucide-style, 24x24 viewBox, stroke-based, `class="w-full h-full"`).

**Verification**: `cd apps/landing && hugo --quiet 2>&1; echo $?` exits 0. Then search the built `public/ai-strategy/index.html` for `path d="M19 14c1.49` to confirm the heart SVG is rendered.

**Rollback**:
- `git checkout -- apps/landing/layouts/partials/slides-icon.html`
...
### Phase 2 — Sequential (requires WU-1)

- WU-2: Update Layout to Read Prompt from Content File + Fix Copy Button

*Note: WU-2 depends on WU-1 because the content file must exist before the layout can reference it. WU-3 and WU-4 are independent of everything else and run in parallel with WU-1.*

## Recovery Strategy

- **Automatic**: Each implementor rolls back and retries once on failure.
- **Dependency failure**: If WU-1 fails, WU-2 cannot run. WU-3 and WU-4 are independent.
- **Global rollback**: `git checkout -- apps/landing/layouts/_default/ai-strategy.html apps/landing/layouts/partials/slides-icon.html` + `rm -f apps/landing/content/ai-strategy-prompt.en.md` for local files. Outline document can be reverted using the previous content preserved in conversation context.
- **Independent failures**: All Phase 1 units are independent. If one fails, the others still execute.
